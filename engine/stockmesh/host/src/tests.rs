...
pub(crate) fn fixture() -> Fixture {
    ...
    for i in 0..2u8 {
        ...
        let mut policy_data = vec![0; 193];
        policy_data[..8].copy_from_slice(b"SKEWSTK1");
        policy_data[104..136].copy_from_slice(cash.as_ref());
        policy_data[136..168].copy_from_slice(mint_key.as_ref());
        policy_data[168..176].copy_from_slice(&1u64.to_le_bytes());
        policy_data[176..184].copy_from_slice(&100u64.to_le_bytes());
        policy_data[184..192].copy_from_slice(&300u64.to_le_bytes());
        policy_data[192] = 17;
        let mut claim_data = vec![0; 288];
        claim_data[..8].copy_from_slice(b"SKEWCLM1");
        let identity = ProductIdentity {
            instrument: "NVDA".into(),
            issuer: format!("FIXTURE_{i}"),
...
fn exact_wallet_setup_prefix_initializes_missing_product_atas_and_nonce() {
    ...
    let assets = f
        ...
        .collect::<Vec<_>>();
    let setup = WalletSetup::plan(owner, &assets, None, read)
        .unwrap()
        .with_nonce(program, 0, read)
        .unwrap();
    ...
    let snapshot = feed.publish(&response).unwrap();
    let wallet_setup_message = setup
        .compile_setup_only(&[table.clone()], [18; 32], 300_000)
        .unwrap()
        .unwrap();
    setup
        .validate_setup_only_message(&snapshot, &wallet_setup_message)
        .unwrap();
    let mut changed_setup_message = wallet_setup_message.clone();
    *changed_setup_message.last_mut().unwrap() ^= 1;
    assert!(setup
        .validate_setup_only_message(&snapshot, &changed_setup_message)
        .is_err());
    let message = compile_unsigned_v0(owner, &instructions, &[table], [19; 32]).unwrap();
    assert!(setup
        .validate_setup_only_message(&snapshot, &message)
        .is_err());
    let mut intent = f.intent.clone();
